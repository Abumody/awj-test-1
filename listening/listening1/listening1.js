...
for (let i = 1; i <= TOTAL_TESTS; i++) {
  ...
  // 🔑 نفس TEST_ID المستخدم داخل listening-test.js
  const TEST_ID = `listening1_test${i}`;
  ...
  title.textContent = `Listening Test ${i}`;
  ...
  if (left <= 0) {
    ...
    button.onclick = () => {
      window.location.href = `listening1_test${i}.html`;
    };
  }
